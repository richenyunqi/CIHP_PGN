...
class Test:
    def __init__(self, path):
        ...
        self.N_CLASSES = 20
        self.DATA_DIR = path
        self.LIST_PATH = path + '/list/val.txt'
        self.DATA_ID_LIST = path + '/list/val_id.txt'
    ...
    def main(self):
        ...
        with tf.name_scope("create_inputs"):
            reader = ImageReader(self.DATA_DIR, self.LIST_PATH,
                                 self.DATA_ID_LIST, None, False, False, False,
                                 coord)
        ...
        raw_output = tf.reduce_mean(tf.stack([parsing_out1, parsing_out2]),
                                    axis=0)
        ...
        raw_output_all = tf.reduce_mean(tf.stack(
            [head_output, tail_output_rev]),
                                        axis=0)
        ...
        weights = tf.cast(
            tf.less_equal(gt, self.N_CLASSES - 1), tf.int32
        )  # Ignoring all labels greater than or equal to self.N_CLASSES.
        mIoU, update_op_iou = tf.contrib.metrics.streaming_mean_iou(
            preds, gt, num_classes=self.N_CLASSES, weights=weights)
